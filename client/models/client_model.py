...
class ClientModel:
    ...
    def ask_user_name(self):
        ...
        user_name = input(self.view.template("ask_for_username.txt").substitute())
    ...
    def ask_server_info(self):
        ...
        server_address = input(
            self.view.template("ask_for_server_info_1.txt").substitute()
        )

        server_port = input(
            self.view.template("ask_for_server_info_2.txt").substitute()
        )

        print(
            self.view.template("ask_for_server_info_3.txt").substitute(
                {"server_host": server_address, "server_port": server_port}
            )
        )
    ...
    def create_chat_room_or_join_prompt(self):
        while True:
            operation = int(
                input(
                    self.view.template("ask_for_operation.txt").substitute(
