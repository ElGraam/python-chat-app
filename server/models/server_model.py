...
class ServerModel:
    ...
    def tcp_accept(self):
        ...
        logging.info(
            self.view.template("tcp_accept_1.txt").substitute({"address": address})
        )
    ...
    def create_room(self, client_connection, client_request):
        if not self.chat_room_list.check_room_name(client_request["room_name"]):
            ...
            self.chat_room_list.set_room_password(
                client_request["room_name"], client_request["password"]
            )
            ...
            client_request["token"] = (
                client_request["user_name"] + ":" + self.generate_token()
            )
